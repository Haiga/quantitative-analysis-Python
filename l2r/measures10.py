...
def modelEvaluation(test, score, nFeatures):
    ...
    mrrQueries = np.array([0.0] * len(listQueries), dtype=float)
    ...
    for query in listQueries:
        ...
        i = 0
        mrr_flag_first_relevance = True
        for doc in matQuery:
            labelQuery[i] = test.y[int(doc[1])]
            i += 1
            if mrr_flag_first_relevance and relevanceTest(dataset, test.y[int(doc[1])]) == 1 and i <= 10:
                mrrQueries[idQ] = 1/i
                mrr_flag_first_relevance = False
    ...
    return ndcgQueries, apQueries, mrrQueries
...
if __name__ == "__main__":
    init = np.array(
        [0.0500, 0.1500, 0.3000, 0.4500, 0.5500, 0.4000, 0.3500, 0.3000, 0.2500, 0.2000, 0.3000, 0.3000, 0.3000, 0.3000,
         0.3000, 0.2500, 0.2500, 0.2500, 0.2500, 0.2500, 0.4000, 0.1500, 0.4000, 0.1500, 0.4000, 0.2000, 0.4500, 0.2000,
         0.4500, 0.2000, 0.2542, 0.2629, 0.2802, 0.2975, 0.3061, 0.2918, 0.2994, 0.3147, 0.3301, 0.3378])
    init = np.reshape(init, (8, 5))
    init = init.T
    print(init)
    geoRisk = getGeoRisk(init, 1)
    print(geoRisk)
    # trisk=tRisk(init[:,1], init[:,2], 1)
    # print trisk
